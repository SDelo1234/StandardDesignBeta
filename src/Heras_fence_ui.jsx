...
export default function HerasInputMockup() {
  ...
  const tryLogin = (e) => {
    ...
    if (validatePin(pin)) {
      setAuthed(true);
      // Always land on the selection page after login
      setModule(null);
      try {
  ...
  const logout = () => {
    setAuthed(false);
    // Reset module so the selector shows on next login
    setModule(null);
    try {
            ...
            <section className="mb-6">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <button
                  type="button"
                  disabled
                  className="group relative overflow-hidden rounded-2xl border p-3 text-left shadow-sm opacity-60 cursor-not-allowed"
                  aria-disabled="true"
                  aria-label="Shafts (coming soon)"
                >
                  <div className="mb-3 h-36 w-full rounded-xl bg-gray-100 flex items-center justify-center text-xs text-gray-500">No image</div>
                  <div className="text-sm font-medium">Shafts</div>
                  <div className="mt-1 text-xs text-gray-600">Coming soon</div>
                  <div className="absolute inset-0 flex items-center justify-center bg-white/60 text-xs font-medium text-gray-700">Coming soon</div>
                </button>
              </div>
            </section>
          ) : null}

          {/* Existing app content (hidden until a module is chosen) */}
          <div className={module === null ? 'hidden' : ''}>
          <div className="mb-3">
            <button
              type="button"
